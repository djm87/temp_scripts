...
def write_spectra(ttheta,intensity,file,sigma=None):
    if sigma is None:
        ...
        np.savetxt(file,
            ...
            delimiter='\t',
            header=f"{file}\nBANK 1 {len(ttheta)} {len(ttheta)} CONS 300.00 0.55 0 0 FXYE")
...
def process_image(file: str):
    ...
    if not filename.parent.is_dir():
        filename.parent.mkdir(exist_ok=True)
    filename = 'test.fxye'
    write_spectra(ttheta,intensity,str(filename),sigma=sigma)
...
if __name__ == "__main__":
    ...
    freeze_support()
    #rebuild_cinema()
    file_watcher(watch_dir,process_all=True)
